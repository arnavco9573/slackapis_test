...
import TaskCreationProgress from "./TaskCreationProgress"
...
interface CreateTaskPartnerModalProps {
    isOpen: boolean
    onClose: () => void
    categories: Category[]
...
const CreateTaskPartnerModal = ({
    isOpen,
    onClose,
    categories,
    defaultCategoryId,
    onSave
}: CreateTaskPartnerModalProps) => {
    ...
    // Reset step and formData when modal opens/changes
    useEffect(() => {
        if (isOpen) {
            setStep(1)
            setMaterialType("")
    ...
    const handleSave = () => {
        ...
        onSave({
            ...
            isLive: true // Default to live for new tasks
        })
        onClose()
    }
...
export default CreateTaskPartnerModal
